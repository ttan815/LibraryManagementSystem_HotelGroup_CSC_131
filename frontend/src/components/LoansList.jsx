...
function LoansList() {
 ...
 const loadBooks = async () =>{ // Function to load the books, but makes it as a hashmap, which allows the book IDs to be indexed easily later to get the book info associated with it.
  ...
  const approveReservation = async (reservationID, book_id, user_id) =>{ // Function that will take the inputs from the field for the reservation and passed in parameters to approve the reervation
        ...
        <table>
            ...
            <tbody>
                ...
                {reservations && reservations.filter(reservation => reservation.status.trim().toUpperCase() !== "CANCELLED").map((reservationObject) =>(
                    ...
                    {/* Actions */}
                    <td>
                        <button type="button" onClick={()=>approveReservation(reservationObject.id, reservationObject.book_id, reservationObject.user_id)} className="loanButtons">Approve</button>
